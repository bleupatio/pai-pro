...
| `generate_video.js` | `/video-compose` | PAI Lite | `jm-video-generation` | ~2–4 min. ~$0.20/sec @ 720p, ~$0.44/sec @ 1080p + ~$0.01/ref preupload. Real money — only after explicit ask. Refs must be publicly fetchable URLs. |
...
- `server/pai_*.js` — PAI Lite clients imported by the CLIs:
  ...
  - **Asset uploads**: `pai_assets_client.js` (`jm-assets` raw; chip-UX cache + event-emitter surface — exports `paiAssetEvents`, `snapshotAssetStates`, `seedAssetCache`, `uploadReferenceUrl`, `preuploadReferenceUrl`, `preuploadCanvasUrl`, `uploadReferences`).
